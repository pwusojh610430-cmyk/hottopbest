...
function LearnPage() {
  ...
  return <><Header/><main className="learn-page">
    ...
    <section className="complete-library expanded-library"><div className="library-sidebar"><span className="section-kicker">ALL ARTICLES</span><h2>Explore the complete library.</h2><p>Browse 57 original guides across modern SEO, AI search, content, authority, and marketing.</p><nav>{Object.entries(articleLibrary).map(([name,items])=><a href={`#library-${articleSlug(name)}`} key={name}>{name}<span>{items.length}</span></a>)}</nav></div><div className="all-library-content">{Object.entries(articleLibrary).map(([name,items])=><section className="library-category" id={`library-${articleSlug(name)}`} key={name}><div className="library-title"><div><span>{name==='Popular'?'EDITOR’S COLLECTION':'TOPIC COLLECTION'}</span><h3>{name}</h3></div><span>{items.length} guides</span></div><div className="library-card-grid">{items.map((article,index)=><button onClick={()=>openLibraryArticle(article)} key={article[0]}><span>{String(index+1).padStart(2,'0')}</span><small>{name}</small><b>{article[0]}</b><p>{article[1]}</p><em>Read guide <ArrowRight/></em></button>)}</div></section>)}</div></section>

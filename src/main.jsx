...
const templateCategories = [
  {name:'SEO Templates',slug:'seo',description:'Plan campaigns, audit important pages, report progress, and align stakeholders around a practical search roadmap.',image:seoStrategyImg,items:['Keyword Research Worksheet','On-Page SEO Brief','SEO Strategy Canvas','90-Day SEO Marketing Plan','SEO Proposal & Scope','Monthly SEO Report','Technical SEO Audit Checklist']},
  {name:'Content Marketing Templates',slug:'content',description:'Turn audience insight into an organized editorial system—from strategy and calendars to briefs, articles, and outreach.',image:contentWorkflowImg,items:['Content Strategy Canvas','Content Marketing Plan','Editorial Calendar','Customer Case Study','Useful Blog Post Brief','Editorial Outreach Email']},
  {name:'Email Marketing Templates',slug:'email',description:'Build focused email journeys that welcome subscribers, explain value, and turn attention into a useful next action.',image:trafficImg,items:['Landing Page Wireframe','Simple Sales Funnel','Welcome Email Sequence','Weekly Newsletter Framework']},
  {name:'Social Media Templates',slug:'social',description:'Plan a consistent social presence, create platform-ready assets, and review performance without chasing vanity metrics.',image:competitorImg,items:['Social Media Strategy','LinkedIn About Section','Short Video Script','YouTube Thumbnail Brief','YouTube Description','Facebook Page Setup','Paid Social Ad Brief','Social Media Audit']},
  {name:'Public Relations Templates',slug:'pr',description:'Organize a credible story, prepare press materials, and pitch journalists with a concise reason their audience should care.',image:backlinksImg,items:['PR Campaign Plan','Press Release Framework','Journalist Pitch Email']},
]
const allTemplates=templateCategories.flatMap(category=>category.items.map(title=>({...category,title,slug:articleSlug(title)})))
const templateHref=title=>`/templates/${articleSlug(title)}`
function templateBody(template){return `${template.title.toUpperCase()}\n\n1. OUTCOME\nWhat should change after this work is complete?\nPrimary business outcome:\nAudience action:\nSuccess metric:\n\n2. AUDIENCE & CONTEXT\nPrimary audience:\nProblem or decision:\nWhat they already know:\nObjections or constraints:\n\n3. CORE INPUTS\nEvidence, examples, or source material:\nKey message or topic:\nRequired links and calls to action:\nOwner and due date:\n\n4. EXECUTION PLAN\nStep 1 — Research and collect inputs.\nStep 2 — Create the first focused draft.\nStep 3 — Review accuracy, clarity, and usefulness.\nStep 4 — Publish, distribute, or deliver.\n\n5. QUALITY CHECK\n[ ] The outcome and audience are specific.\n[ ] Every claim can be supported.\n[ ] The next action is clear.\n[ ] Ownership and timing are defined.\n[ ] Results will be reviewed and documented.\n\n6. NOTES & NEXT ITERATION\nWhat worked:\nWhat needs improvement:\nNext review date:`}

function TemplatesPage(){return <><Header/><main className="templates-page"><section className="templates-hero"><div><span className="section-kicker">FREE MARKETING TEMPLATES</span><h1>The practical marketing<br/>templates library.</h1><p>Original, ready-to-use frameworks for SEO, content, email, social media, and public relations. Copy a template, adapt it to your workflow, and get useful work moving faster.</p><a href="#template-library">Browse 28 free templates <ArrowRight/></a><div><span><Check/> No signup</span><span><Check/> Editable</span><span><Check/> Free to use</span></div></div><img src={seoToolkitImg} alt="Illustration of a marketing templates workspace"/></section><section className="template-intro" id="template-library"><span className="section-kicker">CHOOSE A CATEGORY</span><h2>Everything you need to move<br/>from idea to execution.</h2><p>Each collection includes focused planning documents, checklists, briefs, and reporting frameworks written for real marketing work.</p></section><section className="template-category-list">{templateCategories.map((category,index)=><article id={`templates-${category.slug}`} key={category.name}><div className="template-category-copy"><span>0{index+1}</span><img src={category.image} alt={`Illustration for ${category.name}`}/><small>{category.items.length} RESOURCES</small><h2>{category.name}</h2><p>{category.description}</p><a href={templateHref(category.items[0])}>Start with this collection <ArrowRight/></a></div><div className="template-resource-list"><div><span>WHAT’S INCLUDED</span><b>{category.items.length} ready-to-use resources</b></div>{category.items.map((title,itemIndex)=><a href={templateHref(title)} key={title}><span>{String(itemIndex+1).padStart(2,'0')}</span><b>{title}</b><small>Open template <ArrowRight/></small></a>)}</div></article>)}</section><section className="templates-cta"><div><span className="section-kicker light">BUILD A BETTER WORKFLOW</span><h2>Start with a useful structure.<br/>Make it yours.</h2></div><p>Every template is a starting point. Add your customer insight, evidence, brand voice, and review process to turn the framework into work only your team can create.</p></section></main><Footer/></>}

function TemplateDetailPage({template}){const[copied,setCopied]=useState(false);const body=template?templateBody(template):'';if(!template)return <><Header/><main className="not-found"><span>404</span><h1>We couldn’t find that template.</h1><a href="/templates">Browse all templates <ArrowRight/></a></main><Footer/></>;const categorySlug=templateCategories.find(item=>item.name===template.name)?.slug||'';const copy=async()=>{await navigator.clipboard.writeText(body);setCopied(true);setTimeout(()=>setCopied(false),1500)};const download=()=>{const blob=new Blob([body],{type:'text/plain'});const url=URL.createObjectURL(blob);const link=document.createElement('a');link.href=url;link.download=`${template.slug}.txt`;link.click();URL.revokeObjectURL(url)};const related=allTemplates.filter(item=>item.name===template.name&&item.slug!==template.slug).slice(0,3);return <><Header/><main className="template-detail-page"><section className="template-detail-hero"><div><a href="/templates">Templates</a><span>/</span><a href={`/templates#templates-${categorySlug}`}>{template.name}</a><span className="section-kicker">FREE EDITABLE TEMPLATE</span><h1>{template.title}</h1><p>A practical, original framework for planning the work, aligning contributors, and producing a result you can review and improve.</p><div><button onClick={copy}>{copied?<Check/>:<Copy/>}{copied?'Copied':'Copy template'}</button><button onClick={download}><Download/> Download .txt</button></div></div><img src={template.image} alt={`Illustration for ${template.title}`}/></section><section className="template-detail-layout"><aside><b>How to use it</b><ol><li>Replace each prompt with details from your project.</li><li>Remove sections that do not affect the outcome.</li><li>Assign an owner and review date.</li><li>Save results for the next iteration.</li></ol><a href="/templates">Browse all templates <ArrowRight/></a></aside><article><div className="template-sheet-head"><div><span>HOTTOPBEST TEMPLATE</span><h2>{template.title}</h2></div><button onClick={copy}>{copied?<Check/>:<Copy/>}</button></div><pre>{body}</pre></article></section><section className="template-guidance"><div><span className="section-kicker">MAKE IT USEFUL</span><h2>A template is the beginning,<br/>not the finished work.</h2></div><div><h3>Start with the decision</h3><p>Define the audience, desired action, evidence, and constraint before filling in tactics.</p><h3>Add real information</h3><p>Replace generic prompts with customer language, original examples, named owners, and measurable outcomes.</p><h3>Review the result</h3><p>Record what happened so the next version of the template becomes more useful for your team.</p></div></section>{related.length>0&&<section className="related-templates"><span className="section-kicker">MORE IN THIS COLLECTION</span><h2>Related templates</h2><div>{related.map(item=><a href={templateHref(item.title)} key={item.title}><small>{item.name}</small><b>{item.title}</b><span>Open template <ArrowRight/></span></a>)}</div></section>}</main><Footer/></>}
...
function Footer() {
  return <footer><div className="footer-top"><div><Logo/><p>Practical AI and SEO tools<br/>for smarter, sustainable growth.</p></div><div><b>Tools</b><a href="/ai-tools">AI tools</a><a href="/seo-tools">SEO checker</a><a href="/ai-tools#content-tools">Content tools</a></div><div><b>Resources</b><a href="/learn">Guides</a><a href="/templates">Templates</a><a href="/comparisons">Comparisons</a><a href="/newsletter">Newsletter</a></div><div><b>Company</b><a href="/about">About</a><a href="/contact">Contact</a><a href="/submit-tool">Submit a tool</a></div></div><div className="footer-bottom"><span>© 2026 HotTopBest. Built for better work.</span><span><a href="/privacy">Privacy</a><a href="/terms">Terms</a></span></div></footer>
}

function HomePage(){return <><Header/><main><Hero/><Checker/><Tools/><ArticlesHub/><Workflow/><Learn/><Newsletter/></main><Footer/></>}
function App(){const path=window.location.pathname.replace(/\/$/,'');if(path==='/seo-tools')return <SeoToolsPage/>;if(path==='/ai-tools')return <AiToolsPage/>;if(path==='/learn')return <LearnPage/>;if(path==='/categories')return <CategoriesPage/>;if(path==='/templates')return <TemplatesPage/>;if(path==='/comparisons')return <ComparisonsPage/>;if(path==='/newsletter')return <NewsletterPage/>;if(path==='/about')return <AboutPage/>;if(path==='/contact')return <FormPage type="contact"/>;if(path==='/submit-tool')return <FormPage type="submit"/>;if(path==='/privacy')return <LegalPage kind="privacy"/>;if(path==='/terms')return <LegalPage kind="terms"/>;if(path.startsWith('/templates/'))return <TemplateDetailPage template={allTemplates.find(item=>item.slug===path.split('/').pop())}/>;if(path.startsWith('/tools/'))return <ToolDetailPage tool={tools.find(item=>toolSlug(item.name)===path.split('/').pop())}/>;if(path.startsWith('/articles/'))return <ArticlePage article={getArticleBySlug(path.split('/').pop())}/>;return <HomePage/>}
